  ...
  <MainLayout>
    <div class="space-y-4 sm:space-y-6">
      ...
      <div class="space-y-4 sm:space-y-6">
        ...
        >
          <h3
            class="text-lg sm:text-xl font-semibold text-foreground mb-3 sm:mb-4"
          >
            {{ section.title }}
          </h3>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            ...
            >
              <dt class="text-xs sm:text-sm font-medium text-muted-foreground">
                {{ formatFieldName(field) }}
              </dt>
              <dd class="text-sm sm:text-base text-foreground break-words">
      ...
      <div
        class="pt-4 sm:pt-6 border-t space-y-3 sm:space-y-0 sm:flex sm:justify-between sm:items-center"
      >
        <Button
          variant="outline"
          @click="goBackToForm"
...
function exportData() {
  ...
  const exportFileDefaultName = `cfdp-questionnaire-${
...
function submitForm() {
  ...
  alert(
    "Questionnaire submitted successfullyyy, In a real application, this would send data to laravel if ever there is one."
  );
